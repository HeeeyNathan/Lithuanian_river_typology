...
#====== Load data ======
Unique_sites <- read_excel("Input data/1_site_factors_2009-2023.xlsx") |>
  mutate(
    site_id                = as.factor(site_id),
    data_year              = as.numeric(data_year),
    latitude               = as.numeric(latitude),
    longitude              = as.numeric(longitude),
    river_basin            = as.factor(river_basin),
    catchment_subcatchment = as.character(catchment_subcatchment),
    river_name             = as.character(river_name),
    modification_state     = as.factor(modification_state),
    modification_group     = as.character(modification_group),
    river_type             = as.factor(river_type),
    reference_site         = as.logical(reference_site)
  ) |>
  rename(
    year      = data_year,
    catchment = catchment_subcatchment
  ) |>
  dplyr::select(-id)
...
inset <- ggplot() +
  ...
  theme_void() +
  theme(panel.background = element_rect(fill = "#849AB8"),
